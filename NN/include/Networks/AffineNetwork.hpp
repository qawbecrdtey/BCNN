#ifndef BNN_AffineNetwork_hpp
#define BNN_AffineNetwork_hpp
...
namespace NN {
	...
	*/
	...
	class AffineNetwork {
		...
		std::unique_ptr<RMSProp[]> momentum_bias;
		std::unique_ptr<RMSProp[]> momentum_weight;
...
#define D_TANHMAP [](Eigen::Matrix<double, -1, 1> const &x) -> Eigen::Matrix<double, -1, 1> { Eigen::Matrix<double, -1, 1> o(x); for(std::size_t i = 0; i < x.rows(); i++) { double const t = std::tanh(o(i)); o(i) = 1 - t * t; } return o; }
	...
	public:
		...
		AffineNetwork(std::array<std::size_t, DEPTHP> info)
			: depth(DEPTHP - 1),
			...
			outactfunc(IDMAP),
			hidactfunc(ELUMAP),
			momentum_bias(std::make_unique<RMSProp[]>(DEPTHP - 1)),
			momentum_weight(std::make_unique<RMSProp[]>(DEPTHP - 1))
		{
			for (std::size_t i = 0; i <= depth; i++)
			{
			...
			for (std::size_t i = 0; i < depth; i++)
			{
				...
				momentum_weight[i] = RMSProp(info[i], info[i + 1], 0.001, 0.5);
				momentum_bias[i] = RMSProp(info[i + 1], 1, 0.001, 0.5);
			}
		}
		...
		friend std::ostream &operator<<(std::ostream &os, AffineNetwork const &network)
		{
